...
大于二维的矩阵，被称为张量（Tensor），而每一张图片，一般都是通过三维的 Tensor 来进行存储，这三个维度分别是长度、宽度、通道（channel），而其中的每一个数值，都代表某个二维位置上的某个通道的颜色数值大小。在图像分类的 Model 中，一般输入 Network 的向量，是由张量中各个分量拼接而成的高维向量。
...
Fully Connected Network 中每层 Network 中的每一个 Neuron 针对输入的高维向量中每一个维度的标量都需要有 Weight 参数，如一层 Network 有 1000 个 Neuron，而输入的向量维度为 $100*100*3$，则 Model 一共有 $100*100*3*1000=3*10^7$ 个 Weight 参数。

### Observation-1

在 Image Classification 中，实际上并不需要每个 Neuron 读取整个向量，即并不需要像 Fully Connected 那样设置那么多的 Weight 参数。因为图像通常都具备某种特征，基于这些特征形成对应的模式（Pattern），从而每个 Neuron 只需要关注输入的向量中对应的一部分就可以了，并不需要为每个维度声明 Weight。

### Simplification-1 Typical Setting

在 Network 的高维向量中，被 Neuron 用作输入的部分，被称为 Receptive Filed。

一般同一个 Receptive Filed 会有一组 Neuron 来读取，相邻的 Receptive Field 之间，一般会有一定的重叠部分，每次横向移动的步长被称为 stride，纵向移动的步长被称为。

当 Receptive Field 超出图像张量的范围，则可以通过 Padding 等技术实现补值。
