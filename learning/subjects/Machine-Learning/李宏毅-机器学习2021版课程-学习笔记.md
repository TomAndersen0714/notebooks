...
Network 中的每层 Fully Connected Layer 中的每一个 Neuron 针对输入的高维向量中每一个维度的标量都需要有 Weight 参数，如一层 Network 有 1000 个 Neuron，而输入的向量维度为 $100*100*3$，则 Model 一共有 $100*100*3*1000=3*10^7$ 个 Weight 参数。


### Convolutional Neural Network (CNN)

基于影像独特的特征，专门用于影像处理的 Neural Network。
