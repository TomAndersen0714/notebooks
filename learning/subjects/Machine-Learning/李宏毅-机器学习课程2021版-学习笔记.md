...
一味追求 Interpretable 而忽视 Powerful，就类似于偏执地在路灯下找黑暗中的钥匙。而一味地追求 Powerful ，而忽视 Interpretable，那么模型就无法在工程伦理要求严格的场景中使用。

### Local explanation and global explanation

Local explanation：针对特定的输入和输出，解释输入产生输出的原因，如：为什么你认为这个照片是猫。

Global explanation：针对特定的输出，解释什么样的输入会产生对应的输出，如：你认为什么样的照片是猫。
