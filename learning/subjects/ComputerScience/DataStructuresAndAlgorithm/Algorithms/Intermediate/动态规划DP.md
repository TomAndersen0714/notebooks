...
问题描述：
1. 输入两个字符串，返回两个字符串的最长公共子串对应的长度
解题思路：
1. 动态规划 Dynamic Programming
	1. 假设 `dp[i][j]` 代表两个字符串分别前 i 个和前 j 个字符对应的最大公共子串，则当 i 和 j 字符相同时，则 `dp[i][j]=dp[i-1][j-1]+1`，否则 `dp[i][j]=max{ dp[i-1][j], dp[i][j-1] }`
	2. 初始状态时，当 i=0 或 j=0 时，`dp[i][j]` 都为 0，因为其中一方为空字符串
	3. 从 1 开始遍历，遍历结束时，dp 数组中的最后一个元素即为原题解
