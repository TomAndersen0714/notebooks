...
解题思路： 
1. Two pointers：
	1. 方法：左右指针的初始位置分别为数组的起止位置，当两指针元素数值之和大于目标值，即 $a+b>target$ 时，则右指针左移 1 位，反之则左指针右移 1 位，其中 $target=res[0]+res[1]$。
...
注意事项：
1. 在移动指针时，为了避免结果中出现重复的 tuple，每次指针移动时，如果移动后指针的元素值和之前相同，则需要不断跳过，避免出现重复运算。

解题思路：
1. Two pointers：
	1. 思路：根据三个指针指向的元素和目标值之间的关系，先将数组进行排序，然后遍历数组，假设当前游标指向的元素为结果向量中的最小元素，那么接下来就是判断，游标之后的元素中是否有某两个元素的值之和等于特定值（或满足某特定条件），故而将三数问题，转换为了双数问题，从而应用双指针进行求解。
	2. 空间复杂度：$O(1)$，时间复杂度： $O(n^2)$。 
