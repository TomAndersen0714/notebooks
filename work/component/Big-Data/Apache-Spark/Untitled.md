...
SQL Select 语句中按需取列，显式列出对应列字段名，尽量避免使用 `SELECT *`，即减少 Task 输入的数据量。
...
通过调整相关参数，或者使用 Hint，来触发 Broadcast Join 算法，避免 SortMerge Join 时的 Exchange（Shuffle） 阶段，减少数据 IO，提升性能，避免数据倾斜。
...
2. 调整 spark.sql.shuffle.partitions ：调整 Spark SQL Shuffle 阶段生成的 partition 数 ` spark.sql.shuffle.partitions`，使得每个 Task 处理的数据量分布均匀，避免出现 OOM。
