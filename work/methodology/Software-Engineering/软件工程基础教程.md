...
[软件工程相关术语](work/methodology/Software-Engineering/软件工程相关术语.md)
...
2. **接口设计**：定义模块对外提供的接口和API。
3. **数据结构**：设计模块内部使用的数据结构。
4. **算法**：设计模块内部使用的算法和逻辑。
5. **模块间的依赖**：描述模块之间的依赖关系和调用关系。
6. **功能流程描述**：使用顺序图描述清楚每个功能点，是如何调用各个模块的，避免实现后无法满足需求
